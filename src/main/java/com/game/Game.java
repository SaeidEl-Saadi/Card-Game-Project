...
public class Game {
    ...
    public void dealCards() {
        for (int i = 0; i < 4; i++) {
            for (int k = 0; k < 12; k++) {
                players.get(i).addCard(adventureDeck.remove(adventureDeck.size() - 1));
            }
        }
    }

    public Card drawEventCard() {
        return eventDeck.remove(eventDeck.size() - 1);
    }
    ...
    public void drawAdventureCards(Player p, int amount) {
        for (int i = 0; i < amount; i++) {
            p.addCard(adventureDeck.remove(adventureDeck.size() - 1));
        }
    }
